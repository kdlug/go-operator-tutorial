...
const memcachedFinalizer = "cache.github.com/finalizer"
...
type MemcachedReconciler struct {
	...
	Recorder record.EventRecorder // @TOTO po co ten recorder?
}

// The controller needs certain RBAC permissions to interact with the resources it manages
...
func (r *MemcachedReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if memcached.Status.Conditions == nil || len(memcached.Status.Conditions) == 0 {
		...
		fmt.Printf("NS: %s", req.NamespacedName)
	...
	// The foxllowing implementation will update the status
	meta.SetStatusCondition(&memcached.Status.Conditions, metav1.Condition{Type: typeAvailableMemcached,
...
func (r *MemcachedReconciler) SetupWithManager(mgr ctrl.Manager) error {
	return ctrl.NewControllerManagedBy(mgr). // controller builder
		// specifies the Memcached type as the primary resource to watch
		// For each Memcached type Add/Update/Delete event the reconcile loop will be sent
		// a reconcile Request (a namespace/name key) for that Memcached object.
		For(&cachev1alpha1.Memcached{}).
		// specifies the Deployments type as the secondary resource to watch.
		// For each Deployment type Add/Update/Delete event, the event handler will map each event to a reconcile
		// Request for the owner of the Deployment. Which in this case is the Memcached object for which the Deployment was created.
